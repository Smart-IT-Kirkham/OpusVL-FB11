...
(function($) {
    var mk_shown_with = curry(function(options, index, obj) {
        ...
        // A comparison part can be prefixed with a ~ to negate it, so this
        // function simplifies those tests.
        function cmp(a,b) {
            var neg = b.startsWith('~');
            if (!b) return neg
            if (neg) {
                b = b.substring(1);
                return a != b;
        ...
        function with_test() {
            ...
            if (this.type == 'radio') {
                ...
                if ((with_parts.length == 2 && !cmp(val, with_parts[1]))) {
                    return false;
                }
                if (with_parts.length < 2 || cmp(val, with_parts[1])) {
                    return true;
            ...
            if (with_parts[1]) {
                return cmp($this.val(), with_parts[1]);
            }
            else {
                return !! $this.val();
            }
